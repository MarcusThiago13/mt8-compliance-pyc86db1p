...
//     USING: ((id = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles p   WHERE ((p.id = auth.uid()) AND (p.is_super_admin = true)))))
...
//     USING: ((user_id = auth.uid()) OR (tenant_id IN ( SELECT tenant_users_1.tenant_id    FROM tenant_users tenant_users_1   WHERE (tenant_users_1.user_id = auth.uid()))))
// Table: tenants
